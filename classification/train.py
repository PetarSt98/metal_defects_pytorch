...
if __name__ == "__main__":
    train_dataset = torchvision.datasets.ImageFolder(config.TRAIN_DIR, transform=config.transformations)
    val_dataset = torchvision.datasets.ImageFolder(config.VAL_DIR, transform=config.transformations)
    test_dataset = torchvision.datasets.ImageFolder('./../vae_dataset/test/', transform=config.transformations)
    train_data_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=config.BATCH_SIZE, shuffle=True)
    test_data_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=config.BATCH_SIZE, shuffle=True)
    val_data_loader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size=config.BATCH_SIZE, shuffle=True)
    ...
    model = Model2(3).to(config.DEVICE)
    ...
    for epoch in range(config.EPOCHS):
        ...
        if loss_val < best_model['val_loss']:
            best_model = {'model': Model2(3).to(config.DEVICE), 'val_loss': loss_val}
